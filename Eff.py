...
if uploaded_file is not None:
    if st.sidebar.button("💾 บันทึกไฟล์นี้เป็นค่าเริ่มต้น (Save Default)", use_container_width=True):
        ...
        st.sidebar.success("✅ บันทึกเป็นไฟล์เริ่มต้นเรียบร้อย!")
...
if error:
    ...
    if st.sidebar.button("💾 บันทึกเงื่อนไข (Save Conditions)", use_container_width=True):
        ...
        try:
            ...
            st.sidebar.success("✅ บันทึกเงื่อนไขเรียบร้อยแล้ว!")
        except Exception as e:
    ...
    date_range = st.sidebar.date_input("📅 ช่วงวันที่", value=(min_date_db, max_date_db), min_value=min_date_db, max_value=max_date_db)
    ...
    if isinstance(date_range, tuple) and len(date_range) == 2:
        ...
        start_disp_date = end_disp_date = date_range[0]
        df = df[df['วันที่ผลิต'] == start_disp_date]

    trial_option = st.sidebar.selectbox("🧪 กรองงาน Trial:", ["แสดงทั้งหมด (ไม่ตัด)", "ตัด Part ที่ผลิตเพียง 1 วัน", "ตัด Part ที่ผลิต 1-2 วัน"], index=0)
    part_day_counts = df.groupby('Part')['วันที่ผลิต'].nunique().to_dict()
    df['จำนวนวันผลิตของPart'] = df['Part'].map(part_day_counts)

    cut_days = 0
    if trial_option == "ตัด Part ที่ผลิตเพียง 1 วัน": cut_days = 1
    elif trial_option == "ตัด Part ที่ผลิต 1-2 วัน": cut_days = 2

    if cut_days > 0:
        df = df[df['จำนวนวันผลิตของPart'] > cut_days]

    machine_groups = ['INJ', 'INM', '510', 'VAC', '400T', '300T', '350T']
    selected_groups = st.sidebar.multiselect("⚙️ กลุ่มเครื่องจักร", options=machine_groups, default=[])
    if selected_groups:
        pattern = '|'.join(selected_groups)
        df = df[df['Machine'].str.contains(pattern, case=False, na=False)]

    selected_machines = st.sidebar.multiselect("🚜 รายเครื่องจักร", options=sorted(df['Machine'].unique()), default=[])
    if selected_machines:
        df = df[df['Machine'].isin(selected_machines)]

    # --- แสดงผลหน้า Dashboard ---
    days_count = (end_disp_date - start_disp_date).days + 1
    st.success(f"📅 **ช่วงวันที่แสดงผล:** {start_disp_date.strftime('%d/%m/%Y')} ถึง {end_disp_date.strftime('%d/%m/%Y')} ({days_count:,} วัน)")
    ...
    st.subheader("📊 เปรียบเทียบยอดผลิตจริง กับ เป้าหมาย")
